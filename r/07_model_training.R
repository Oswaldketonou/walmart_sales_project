...
# Function to train + evaluate RF model
train_rf_model <- function(sample_size, num_trees = 200) {
  ...
  rmse  <- sqrt(mean((rf_pred - eval_actual)^2))
  mae   <- mean(abs(rf_pred - eval_actual))
  mape  <- mean(abs((rf_pred - eval_actual) / eval_actual)) * 100
  smape <- smape(eval_actual, rf_pred)
